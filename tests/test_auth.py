...
class TestCustomAuthPositive(unittest.TestCase):
    ...
    def push_to_ctx(self):
        ...
        self.ctx = self.app.test_request_context(method="POST", data=request_data)
        self.ctx.push()
        log_request()
    ...
    def test_auth_without_credentials(self):
        ...
        self.push_to_ctx()
        auth.login_required(self.f)()
        self.assertEqual(auth.username, anonymous.username)

    def test_auth_existing(self):

        self.set_auth_credentials(username=anonymous.username, token=anonymous.password)
        self.push_to_ctx()
        auth.login_required(self.f)()
        self.assertEqual(auth.username, anonymous.username)
    ...
    def test_auth_not_existing_user(self):
        ...
        self.set_auth_credentials(username="not_existing_user")  # Token doesn't matter
        self.push_to_ctx()
        resp = auth.login_required(self.f)()
